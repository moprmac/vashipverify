...
class PlexShipInfo
{
	...
	protected $plexObj;
		...
		public function AddRecord($plexSN, $partNum, $partQty)
		{
			...
			// Begin section
			// Comment out if this doesn't work tomorrow
			if ( preg_match('/^[sS]{2}[0-9]*\b/', $plexSN) )
			{
				$plexSN = substr($plexSN, 1, strlen($plexSN));
			}
			// end section
		...
		private function ValidAlternate($plexID, $partID)
		{
			...
			{
				throw new Exception("Error:  Unable to find valid part alternatives for Plex Record $plexID");
			}
			
			if ( !preg_match('/^\d+$/',$partID) or is_null($partID) or $partID == "" )
			{
				throw new Exception("Error:  Unable to find valid part alternatives for Part ID $partID");
			}
		...
		public function AddPart($shippingID, $partInfo)
		{
			...
			*/
			...
			$decodedPart = false;
			$shippingInfo = $this->GetShippingStatusByID($shippingID);
			$plexInfo = $this->plexObj->GetStatusByID($shippingInfo['PlexRecord']);
			if ($stmt->execute(array($plexInfo["PartID"])))
			{
				// If there are no altenrates, primary Regex
				if ($stmt->rowCount() == 0)
				{
					$stmt2 = $this->db->prepare("SELECT RegularExpression FROM Regex T1 INNER JOIN PartRegex ON ( Regex.RegexID = PartRegex.RegexID ) WHERE PartRegex.PartID = ?");
					if ($stmt2->execute(array($plexInfo["PartID"])))
					{
						$row = $stmt2->fetch();
					...
					{
						throw new Exception("Error:  Unable to query for regular expression.");
					}
				}
				else
				{
				...
				if ($decodedPart !== false)
				{
					...
					// Is the PartID for the decoded part either the same as or a valid alternate part?
					if ($partID == $plexInfo["PartID"])
					{
						$match=1;
					}
					elseif($this->ValidAlternate($shippingInfo['PlexRecord'], $partID))
					{
						$match=1;
					}
					else
					...
					/*
					$validpart = false;
					if ($partID == $plexInfo["PartID"] || $this->ValidAlternate($shippingInfo['PlexRecord'], $partID))
					{
						$validpart = true;
					}
					...
					*/
					...
					if ($stmt->execute(array($shippingID, $_SESSION['user']['UserID'], date('Y-m-d H:i:s'), $serialNum, $partID, $match, $duplicate, $valid)))
					{
						...
						{
							// Not locking record anymore, just printing DONOTSHIP on validate page
						...
						return(array("InsertID" => $lastInsertID, "ValidPart" => $validPart));
					}
					else
					{
				...
				{
					throw new Exception("Error:  Unable to find a matching part.");
				}
			}
			/********/
		}
